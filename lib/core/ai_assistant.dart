...
class AiAssistant {
  ...
  static String noteContext(Note note, VaultIndex index) {
    final related = {...?index.outgoing[note.path], ...?index.backlinks[note.path]}
        .map((p) => index.notes[p]?.title)
        .whereType<String>()
        .toList();
  ...
  Stream<String> summarize(Note note, VaultIndex index) => client.streamText(
        ...
        messages: [
          (
            role: 'user',
            content: '${noteContext(note, index)}\n'
  ...
  Future<List<({String question, String answer})>> generateFlashcards(Note note, VaultIndex index,
      {int count = 8}) async {
