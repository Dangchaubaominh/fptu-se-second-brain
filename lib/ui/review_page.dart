...
class _ReviewPageState extends ConsumerState<ReviewPage> {
  ...
  Set<String>? _deckPaths(VaultIndex index) =>
      _deck == null ? null : {_deck!, ...?index.outgoing[_deck]};
  ...
  Widget build(BuildContext context) {
    ...
    final decks = index.courses.map((c) {
      final paths = {c.path, ...?index.outgoing[c.path]};
      return (c, all.where((x) => paths.contains(x.notePath)).length, due.where((x) => paths.contains(x.notePath)).length);
        ...
        Padding(
          ...
          child: Card(
            ...
            child: ListTile(
              ...
              subtitle: nextDue == null ? null : Text('Thẻ tiếp theo đến hạn: ${nextDue.day}/${nextDue.month} ${nextDue.hour}:${nextDue.minute.toString().padLeft(2, '0')}'),
            ),
          ),
        ),
      Padding(
