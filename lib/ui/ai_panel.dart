...
class _AiPanelState extends ConsumerState<AiPanel> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _append(String text, String heading) async {
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Đã ghi vào mục "$heading" của note')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(
        child: ListView(controller: _scroll, padding: const EdgeInsets.fromLTRB(12, 4, 12, 12), children: [
          if (_summary != null)
            _ResultCard(
              title: 'Tóm tắt',
              actions: [
                TextButton.icon(
                  onPressed: _busy == null && _summary!.isNotEmpty ? () => _append(_summary!.trim(), 'Tóm tắt AI') : null,
              ...
              actions: [
                TextButton.icon(
                  onPressed: _picked.isEmpty
                      ? null
                      : () {
                          final text = [for (final i in _picked.toList()..sort()) formatFlashcard(_cards![i].question, _cards![i].answer)]
                              .join('\n');
          ...
          if (_chat.isNotEmpty) ...[
            ...
            for (final m in _chat) _Bubble(message: m),
          ],
