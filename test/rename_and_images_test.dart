...
Note note(String path, String content) =>
    Note.parse(path, content, DateTime(2026));

void main() {
  group('replaceLinkTargets', () {
    test('keeps heading, alias and embed marker; skips code blocks', () {
      const src =
          '[[OOP]] [[OOP#Kế thừa|kế thừa]] ![[OOP]] [[Other]]\n```\n[[OOP]]\n```';
  ...
  group('planRename', () {
    ...
    test('rewrites every link to the note except alias links', () {
      ...
      expect(
        plan.updates['Concepts/MVC.md'],
        'Dựa trên [[Concepts/Lập trình OOP#Đa hình]]',
      );
      expect(
        plan.updates['Concepts/Lập trình OOP.md'],
        contains('Xem [[Concepts/Lập trình OOP]] và [[MVC]]'),
      );
    ...
    test('uses the bare title when it is unique', () {
      ...
      expect(
        plan.updates['Concepts/OOP.md'],
        contains('[[Model View Controller]]'),
      );
      expect(
        plan.updates['Home.md'],
        'Không liên quan [[Model View Controller]]',
      );
    });
  });
  ...
  group('images', () {
    test('image embeds render as images with width and are not note links', () {
      expect(
        obsidianToMarkdown('![[sơ đồ.png|300]]'),
        '![300](vaultimg:s%C6%A1%20%C4%91%E1%BB%93.png)',
      );
      expect(
        obsidianToMarkdown('![[Other note]]'),
        '[Other note](wikilink:Other%20note)',
      );
    ...
    test(
      'resolveAttachment: exact path, relative to note, then by file name',
      () {
        final index = VaultIndex(
          '/v',
          const [],
          attachments: ['attachments/uml.png', 'Courses/img/erd.jpg'],
        );
        expect(index.resolveAttachment('uml.png'), 'attachments/uml.png');
        expect(
          index.resolveAttachment('attachments/UML.png'),
          'attachments/uml.png',
        );
        expect(
          index.resolveAttachment('img/erd.jpg', fromFolder: 'Courses'),
          'Courses/img/erd.jpg',
        );
        expect(
          index.resolveAttachment(
            '../Courses/img/erd.jpg',
            fromFolder: 'Concepts',
          ),
          'Courses/img/erd.jpg',
  ...
  test(
    'repository indexes images, imports with unique names and renames files',
    () async {
      ...
      final src = File(
        p.join(
          dir.path,
          '..',
          'fptu_src_${DateTime.now().microsecondsSinceEpoch}.png',
        ),
      );
      ...
      expect(
        await repo.importAttachment(src.path),
        startsWith('attachments/fptu_src_'),
      );
      ...
      expect(
        () => repo.renameFile('B.md', 'B.md'),
        throwsA(isA<FileSystemException>()),
      );
    },
  );

  group('safe persistence', () {
    test(
      'writeNote detects an external edit and preserves the disk version',
      () async {
        final dir = await Directory.systemTemp.createTemp('fptu_conflict');
        addTearDown(() => dir.delete(recursive: true));
        final repo = VaultRepository(dir.path);
        await repo.writeNote('A.md', 'ban dau');
        await File(p.join(dir.path, 'A.md'))
            .writeAsString('sua tu Obsidian', flush: true);

        await expectLater(
          repo.writeNote('A.md', 'sua trong app', expectedContent: 'ban dau'),
          throwsA(
            isA<NoteConflictException>()
                .having((e) => e.path, 'path', 'A.md')
                .having((e) => e.diskContent, 'diskContent', 'sua tu Obsidian'),
          ),
        );
        expect(
          await File(p.join(dir.path, 'A.md')).readAsString(),
          'sua tu Obsidian',
        );
        expect(
    ...
    test(
      'trash preserves the original path and restore never overwrites',
      () async {
        final dir = await Directory.systemTemp.createTemp('fptu_trash');
        addTearDown(() => dir.delete(recursive: true));
        final repo = VaultRepository(dir.path);
        await repo.writeNote('Concepts/A.md', 'ban cu');
        await repo.trashNote('Concepts/A.md');
        final entries = await repo.listTrash();
        expect(entries, hasLength(1));
        expect(entries.single.originalPath, 'Concepts/A.md');

        await repo.writeNote('Concepts/A.md', 'ban moi');
        final restored = await repo.restoreTrash(entries.single);
        expect(restored.path, 'Concepts/A (khôi phục 1).md');
        expect(restored.content, 'ban cu');
        expect(
          await File(p.join(dir.path, 'Concepts', 'A.md')).readAsString(),
          'ban moi',
        );
  ...
  test('VaultNotifier.rename moves the file, updates links and keeps review history', () async {
    ...
    await repo.saveSrs({
      'Concepts/OOP.md|OOP là gì?': const CardState(interval: 6, reps: 2),
    });
    ...
    final r = await c
        ...
        .rename('Concepts/OOP.md', 'Lập trình hướng đối tượng');
    expect(r.path, 'Concepts/Lập trình hướng đối tượng.md');
    expect(r.linkedNotes, 1);
    expect(
      await File(p.join(dir.path, 'Concepts', 'OOP.md')).exists(),
      isFalse,
    );
    expect(
      await File(p.join(dir.path, 'Courses', 'PRO192.md')).readAsString(),
    ...
    expect(
      () => c.read(vaultProvider.notifier).rename(r.path, 'Tên/không hợp lệ'),
